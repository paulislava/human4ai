...
Токен Яндекса (нужен лишь для озвучки) установщик получить сам не может — его
выдаёт OAuth в браузере. Поэтому он открывает страницу выдачи и просит вставить
значение: достаточно скопировать адресную строку целиком, токен из неё
вытаскивается сам. ID приложения запоминается в `.env`
(`YANDEX_OAUTH_CLIENT_ID`), чтобы в следующий раз не искать. Сразу после этого
токен проверяется запросом к glagol — «не тот скоуп» видно на месте, а не когда
колонка промолчит.
