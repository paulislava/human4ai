...
/**
 * Вебхук навыка Алисы «Ответь коду» — голосовая половина вопросов человеку.
 *
 ...
 */
const INVOCATION =
  /^\s*(алиса[\s,]+)?(?:(ответь|отвечу|отвечай|скажи|передай|сообщи|напиши)\s+)?(?:код(?:у|е)?|клоду|кложу)(?=[\s,.:!?…-]|$)[\s,.:!?…-]*/i;

const HELP =
  'Здесь отвечают сессиям Клода. Скажите: ответь коду, и дальше ваш ответ. ' +
  'Ещё умею: повтори, пропусти, сколько вопросов.';
...
export function createAliceRouter(asks: { store: AskStore; service: AskService }): Router {
  ...
  router.post('/alice/:secret', (req: Request, res: Response) => {
    ...
    const utterance = raw.replace(INVOCATION, '').trim().replace(/^[\s,.!?…-]+|[\s,.!?…-]+$/g, '');
