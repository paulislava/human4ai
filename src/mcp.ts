...
const TOOLS = [
  {
    ...
    description:
      ...
      'вопрос произносит Яндекс-Станция, Павел отвечает голосом; ' +
  ...
  },
  {
    ...
    description:
      ...
      'Опрашивай пачками: не больше 5 вызовов подряд, после каждой пачки коротко ' +
      'напиши пользователю «жду ответа» — так видно, что сессия жива, и он может ' +
      'вмешаться, если передумал. Если пользователь написал в сессию сам — ' +
      'немедленно прекрати опрос, вызови cancel_ask и работай по тексту из ' +
      'терминала: он главнее голосового ответа.',
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string' },
  ...
  },
  {
    ...
    title: 'Снять вопрос',
    description:
      'Убрать свой вопрос из очереди: нашёл ответ сам, передумал или — главное — ' +
      'пользователь написал в сессию сам. Написанное в терминал отменяет ожидание: ' +
      'вопрос снимается с очереди (и с колонки), дальше работаем по тексту из чата.',
