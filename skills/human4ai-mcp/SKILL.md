...
## Секреты: значение не попадает в контекст

`ask_user` с `kind: "secret"` (или `"code"`) всегда уходит в Telegram и возвращает
**одноразовую ссылку**, а не значение:

```json
{ "status": "answered", "secret": { "url": "https://…/api/secret/<токен>", "expiresInSec": 1800 } }
```

Забирай значение так, чтобы оно не оказалось ни в выводе, ни в твоём ответе:

```bash
curl -s "<url>" -o ~/.config/some-app/token     # сразу в файл
gh secret set NPM_TOKEN --body "$(curl -s '<url>')"   # или прямо в команду
```

Ссылка срабатывает **один раз** и живёт полчаса; после выдачи значение стирается и
с сервера. Никогда не делай `echo`, `cat` или `curl` без `-o` — вывод попадёт в
контекст, и смысл теряется. Значение в своём ответе пользователю не повторяй.
